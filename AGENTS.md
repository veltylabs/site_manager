...
Este módulo es el **registro de inquilinos** de `misitio`, el panel con el que
los clientes de Velty administran sus sitios web (`misitio.velty.cl`).

- **Es el dueño del `site_id`.** Todos los demás módulos del producto se filtran
  por él; aquí es donde nace. `MemberOf(userID, siteID) (Role, bool)` es la
  **única** forma de responder "¿puede este usuario tocar este sitio?" — si un
  consumidor improvisa la suya, es un defecto de este módulo por no haberla
  expuesto bien.
- **`Site.Status` tiene `draft` como zero value**, y `AccessRequest.Status`
  tiene `pending`. No escribir nada tiene que ser el estado seguro: un sitio que
  nadie tocó no está publicado *por construcción*, no porque alguien puso un
  booleano.
- **`AccessRequest` no es autorización, es un embudo de venta.** Registra a
  quien pide acceso; **nunca** crea una membresía. Dar acceso es una decisión
  humana de Velty, no una consecuencia de haber iniciado sesión con Google.
- **`Site.DomainExpiresAt` existe porque Velty renueva los dominios de sus
  clientes.** Un dominio vencido tumba un sitio sin que nadie haya tocado nada,
  así que la fecha tiene que ser un dato consultable, no un recordatorio en la
  cabeza de alguien.
- **`Plan` sólo lleva límites**, nunca precios ni ciclos de facturación. Cuando
  exista cobro, será un módulo aparte que escribe este campo.
- Este módulo compila **dentro de un Worker de Cloudflare con TinyGo**: sin
  `map[K]V`, sin `encoding/json`, sin `reflect`, sin stdlib pesada. Ver el plan.
